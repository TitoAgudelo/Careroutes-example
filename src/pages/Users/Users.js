...
import './Users.scss'
import { tableStyle, headTableThStyle, headTableTrStyle, headTableStyle, bodyTableTdStyle, bodyTrGroupStyle } from './Users.style'
...
class UsersContainer extends Component {
  ...
  handleAddress({ result }) {
    ...
    this.setState({ address: Object.assign({}, location) })
  }
  ...
  handleSubmit(e) {
    ...
    const { fullname, phone, email, license, regionId, address, role } = this.state

    if (fullname && phone && email && license && regionId && address.addressLine1) {
      const formProps = { name: fullname, phone: phone, email: email, driverLicense: license, role: role, regionId: regionId, address }
  ...
  render() {
    ...
    const { address, fullname, phone, email, license, role, regionId, drivers, regions, submitted, usersFilter, isActive, isLoading } = this.state
          ...
          <div className="container-fluid">
            <div className="row">
              <div className="col-xs-12">
                <div className="section-users-head">
                  <h1 className="section-users-head-title">Users</h1>
                  <button className="button-primary" onClick={this.toggleModal}><span className="text">New User</span></button>
                </div>
                {
                  isLoading ? (
                    <Loading />
                ...
                <Modal title="Add New Driver" show={this.state.isOpen} onClose={this.toggleModal} onAction={this.submitCreateDriver} onSubmit={this.handleSubmit}>
                  { errorMessage ? (
                  ...
                  <form className="care-form" onSubmit={this.handleSubmit}>
                    ...
                    <div className="care-form-group">
                      <div className="care-form-group-item">
                        <label className="care-form-group-item-label">Role</label>
                      </div>
                      <div className="care-form-group-item">
                        <select className="care-form-group-item-select" value={role} name="role" onChange={(e) => this.handleChange(e)} required>
