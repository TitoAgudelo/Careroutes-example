...
class UserDetailContainer extends Component {
  ...
  render() {
    ...
    const { currentDriver, currentEvent, isLoading, isEditing, isCurrentView, isOpen, submitted, events, regions, currentRegion, openRecurrence, role } = this.state;
    const localizer = BigCalendar.momentLocalizer(moment)
    const roles = [ {id: 'admin', name: 'Admin'}, {id: 'director', name: 'Director'}, {id: 'manager', name: 'Manager'}, {id: 'driver', name: 'Driver' }, {id: 'provider', name: 'Provider'} ]
              ...
              <div className="col-xs-12">
                ...
                { currentDriver && currentRegion && regions.length ? (
                  <div className="driver-detail">
                    <h1 className="driver-detail-title">User Detail / {currentDriver.name}</h1>
                    ...
                    {isCurrentView === 'driver' && (
                      <div className="driver-detail-body">
                        <form className="driver-detail-body-form">
                          ...
                          { currentFieldName === 'role' ? (
                            <div className="driver-detail-body-form-group editing">
                              <EditSelectField fieldId={currentDriver.role} fieldLabel={'Role'} list={roles}
                                saveField={this.saveDriver} fieldCancel={this.cancelCurrentField} fieldName="role" />
                            </div>
                            ) : (
                            <div className="driver-detail-body-form-group" onClick={() => this.toggleEditField('role')}>
                              <FieldGroup fieldInfo={currentDriver.role} fieldLabel={'Role'} />
                            </div>
                            )
                          }
...
export default withRouter(connect(mapStateToProps, actions)(UserDetailContainer));
